...
export class ProductsService {  
  ...
  products: Array<Product> = [
    {
      ...
      validRegistration: true,
      completed: true,
      description:'Carne resfriada de bovino sem osso, Picanha maturada ',
  ...
  getproductByEmailAndPassword(email:string|undefined, password:string| undefined ){
    return this.products.find((product) => product.email === email && product.password === password);
  ...
  // createClient(product: Product) {
  //   this.products.push(product);
  //   return this.products;//mexer aqui para cliente
  // }
  ...
  removeByID(id:number){
    const alunosIndex = this.products.findIndex((product) => product.id === id);
    return this.products.splice(alunosIndex,1); 
  }
}
